...
void TestColorConv( IVidiatorImageResize *pControl, CTransformFilter *pFilter, CMediaType *pMTIn, CMediaType *pMTOut, VidiMedia *pIn, VidiMedia *pOut )
{
	...
	printf("Starting ColorConv filter test\n");
	printf("Expected input buffer format is RGB24, output IYUV \n");
	...
	//set our filter type
	hr = pControl->SetFilterType( VIDEOTrans_ColorConv );
	if( hr != S_OK )
	...
	pIn->SetSize( InWidth * InHeight * 3 ); //it's RGB not IYUV
	pOut->SetSize( OutWidth * OutHeight * 3 / 2 );	//this is IYUV
	...
	//this filter is prone to calculation errors, we can hardly rely on the implementation
	for( int i = 0; i < pIn->GetSize() / 3; i+=3 )
	{
		pBuffIn[i+0] = 255 / 3 * 1;		
		pBuffIn[i+1] = 255 / 3 * 2;		
		pBuffIn[i+2] = 255 / 3 * 3;		
	...
	int LumaShiftOut = 0;
	int ErrorCounterY = 0;
	int SamepleY = pBuffOut[ LumaShiftOut ];
	for( int y=0;y<OutHeight;y++)
		for( int x=0;x<OutWidth;x++)
			if( SamepleY != pBuffOut[ LumaShiftOut + y * OutWidth + x ] )
			{
...
//				break;
	...
	int ErrorCounterU = 0;
	int SamepleU = pBuffOut[ UShiftOut ];
	for( int y=0;y<OutHeight / 2;y++)
		for( int x=0;x<OutWidth / 2;x++)
			if( SamepleU != pBuffOut[ UShiftOut + y * OutWidth / 2 + x ] )
			{
//				printf("First Unexpected mismatch at pos %d - %d\n",LumaShiftIn + y * InWidth + x, LumaShiftOut + y * OutWidth + x );
//				break;
				ErrorCounterU++;
	...
	int ErrorCounterV = 0;
	int SamepleV = pBuffOut[ VShiftOut ];
	for( int y=0;y<OutHeight / 2;y++)
		for( int x=0;x<OutWidth / 2;x++)
			if( SamepleV != pBuffOut[ VShiftOut + y * OutWidth / 2 + x ] )
			{
//				printf("First Unexpected mismatch at pos %d - %d\n",LumaShiftIn + y * InWidth + x, LumaShiftOut + y * OutWidth + x );
//				break;
				ErrorCounterV++;
	...
	if( hr != S_OK )
		printf("Could not stop filter \n");
	printf("Done testing ColorConv filter\n");
	printf("=============================================================\n");
}